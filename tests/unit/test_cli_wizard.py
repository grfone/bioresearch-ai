...
# ---------------------------------------------------------------------------
# Regression: --skip-gui must set config.proceed = True
# ---------------------------------------------------------------------------
# A previous version of ``bootstrap.py`` populated a ``GuiConfig``
# from the existing ``.env`` file in the ``--skip-gui`` path but
# never set ``proceed=True``. The
# ``if not config.proceed`` guard at the bottom of
# ``_gui_collect_config`` then cancelled setup with ``"Setup was
# cancelled. Run bootstrap.py again to retry."`` — even though
# the user had explicitly asked the bootstrap to use the existing
# ``.env`` via ``--skip-gui``. The fix extracts the skip-gui path
# into ``load_config_from_env(conda_channel)`` and sets
# ``proceed=True`` unconditionally. These tests guard the
# invariant so the regression can't return.


def test_load_config_from_env_sets_proceed(
    monkeypatch: pytest.MonkeyPatch, tmp_path
) -> None:
    """load_config_from_env must set ``proceed=True`` so the
    ``if not config.proceed`` guard at the bottom of
    ``_gui_collect_config`` doesn't cancel setup.

    Without ``proceed=True`` the bootstrap exits with
    ``"Setup was cancelled. Run bootstrap.py again to retry."``
    even when the user explicitly asked for ``--skip-gui``.
    """
    import bootstrap

    # Pretend ``.env`` lives in tmp_path.
    ...
    config = bootstrap.load_config_from_env()
    assert config.proceed is True, (
        "load_config_from_env must set proceed=True so the "
        "skip-gui path doesn't trigger the 'Setup was cancelled' "
        "exit at the bottom of _gui_collect_config"
    )


def test_load_config_from_env_populates_llm_provider(
    monkeypatch: pytest.MonkeyPatch, tmp_path
) -> None:
    """The provider slug from the .env must reach the config."""
    import bootstrap
...
def test_load_config_from_env_passes_through_pubmed_email(
    monkeypatch: pytest.MonkeyPatch, tmp_path
) -> None:
    """The PubMed email from .env must reach the config."""
    import bootstrap

    env_file = tmp_path / ".env"
    env_file.write_text(
        "DEFAULT_LLM_PROVIDER=openai\n"
        "PUBMED_EMAIL=research@hospital.example\n"
    )
    monkeypatch.setattr(bootstrap, "ENV_FILE", env_file)

    config = bootstrap.load_config_from_env()
    assert config.pubmed_email == "research@hospital.example"


def test_load_config_from_env_skips_blank_lines_and_comments(
    monkeypatch: pytest.MonkeyPatch, tmp_path
) -> None:
    """Robust .env parsing — blank lines and comments must not
    crash the loader. The most common case is a trailing
    newline or a comment block at the top of the file."""
    import bootstrap
...
def test_load_config_from_env_handles_missing_optional_keys(
    monkeypatch: pytest.MonkeyPatch, tmp_path
) -> None:
    """A minimal .env with only the required fields must still
    produce a valid GuiConfig. The defaults inside the loader
    fill in the gaps so the user can cycle through the
    bootstrap with just ``DEFAULT_LLM_PROVIDER`` and
    ``PUBMED_EMAIL`` set."""
    import bootstrap
    ...
    config = bootstrap.load_config_from_env()
    assert config.api_key == ""  # No API_KEY set; user can fix later.
    assert config.base_url == ""
    assert config.selected_local_model is None
    # The build target falls back to ``minimal`` (the default)
    # when ``BUILD_TARGET`` is unset.
    assert config.build_target == "minimal"
    assert config.proceed is True


def test_load_config_from_env_respects_build_target(
    monkeypatch: pytest.MonkeyPatch, tmp_path
) -> None:
    """``BUILD_TARGET=local`` in the .env must propagate to the
    config so the subsequent build phase picks the heavy image."""
    import bootstrap
    ...
    config = bootstrap.load_config_from_env()
    assert config.build_target == "local"
    assert config.proceed is True
