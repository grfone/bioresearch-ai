...
def write_env_file(
    ...
    lines.extend(
        [
            ...
            "",
            "# Abstract enrichment (HTML meta-tag fallback)",
            "# When enabled, the IdentifierResolver falls back to",
            "# scraping the publisher's landing page for the abstract",
            "# when CrossRef/OpenAlex don't have it. Covers DOIs that",
            "# only have the abstract on the publisher's HTML page",
            "# (e.g. Springer book chapters, where CrossRef returns no",
            "# abstract and OpenAlex's abstract_inverted_index is None).",
            "# Adds ~1-2s per DOI lookup. Set to 'false' to disable.",
            "ABSTRACT_ENRICHER_ENABLED=true",
            "",
            "# PDF upload cap",
            "# Maximum size, in bytes, for a PDF dropped on the workspace",
            "# page. The default is 200 MB -- enough for annotated",
            "# reviews, scanned theses, and most Springer/Elsevier/EBSCO",
            "# PDFs. The route enforces a hard ceiling of 200 MB",
            "# (``_PDF_UPLOAD_MAX_BYTES_HARD_CAP`` in",
            "# ``app/api/routes/workspace_actions.py``) so a misconfigured",
            "# env var cannot bypass the safety belt. Set this env var",
            "# only to LOWER the cap (e.g. 50 MB for a multi-tenant",
            "# deployment where users shouldn't upload huge files);",
            "# the hard ceiling remains 200 MB regardless.",
            "# PDF_UPLOAD_MAX_BYTES=209715200",
            "",
            "# Database",
